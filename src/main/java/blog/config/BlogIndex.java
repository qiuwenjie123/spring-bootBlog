...
public class BlogIndex {
     ...
     * @throws IOException
     */
    public void addIndex(TBlog blog) throws IOException {
        //获取写索引对象
        ...
        document.add(new TextField("content",blog.getText(), Field.Store.YES));
        //索引文档加入到写索引中
     ...
     * @throws IOException
     */
    public void updateIndex(TBlog blog) throws IOException {
        //获取写索引对象
        IndexWriter indexWriter = getIndexWriter();
        //实例化索引文档
        Document document = new Document();
    ...
    public List<TBlog> searchBlog(String q) throws Exception{
        ...
        //用来封装查询到的博客
        List<TBlog> blogIndexList = new LinkedList<TBlog>();
        //遍历查询结果
        for(ScoreDoc score : hits.scoreDocs) {
            Document doc = indexSearcher.doc(score.doc);
            TBlog blog = tBlogMapper.selectByPrimaryKey(Integer.parseInt(doc.get("id")));
            /*
            blog.setId(Integer.parseInt(doc.get("id")));
            blog.setDate(doc.get("releaseDate"));
